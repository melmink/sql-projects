with
driver_day as(-- logic to show duration between the first 'Service Appointment Actual End' and the first 'Service Appointment Actual Start' per driver per day
    select
        sr.name,
    ...
    left join salesforce_prod.assigned_resources ar
        on ar.service_appointment_id = sa.id
    left join salesforce_prod.service_resources sr
...
last_resource as ( --logic to identify the latest resource assigned for appointments that have had more than one assigned
    select
