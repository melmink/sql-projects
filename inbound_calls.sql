...
from dbt_gold.rpt_cj_salesforce_calls
...
with outbound as (with calls as (
    select
        s.opportunity_id,
        s.date,
        count(distinct t.id) as outbound_calls
    from "dev"."dbt_gold"."rpt_cj_salesforce_snapshot" as s
    inner join "dev"."salesforce_prod"."tasks" as t
...
from salesforce_prod.sent_envelopes
where created_date > '2021-12-31'
    and sent_via_c = 'Shippo'
;

select count(id)
from salesforce_prod.documents
where created_date >'2021-12-31'
;

-- Doc count extremely low, 70
-- select *
-- from docenginedb_deal.docs
...
select count(distinct id)
from dbt_gold.rpt_cj_salesforce_cases
where created_date > '2021-12-31'
;

select count(salesforce_id)
from dbt_gold.rpt_cj_salesforce_payments
where created_date > '2021-12-31'
;
...
with lifecycle as
    ...
    left join salesforce_prod.tasks t
        on t.what_id = o.id
